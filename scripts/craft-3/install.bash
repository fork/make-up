...
if [ ! "$IDENT_CRAFT_3" = true ]; then
  ...
  if [ "$answer" != "${answer#[Yy]}" ]; then
    if [ "$IDENT_DOCKER" = true ]; then
      ...
      echo
      docker-compose exec "${DOCKER_CRAFT_SERVICE:-php}" composer create-project craftcms/craft /var/www/html
      docker-compose exec "${DOCKER_CRAFT_SERVICE:-php}" composer --working-dir=/var/www/html require --dev squizlabs/php_codesniffer
    else
      echo
